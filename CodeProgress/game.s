...
draw_left:
    ...
    ;    dec SCREEN_POS_LO
   ;     BNE skip_decrement_color_hi  ; If no carry, skip increment
  ;      dec SCREEN_POS_HI             ; Increment COLOR_POS_HI if carry is set
...
dec_hi_screen_byte:
        lda #$FF                    ; Set low byte to $EF if it underflowed
        sta SCREEN_POS_LO
        dec SCREEN_POS_HI 
        rts

dec_hi_color_byte:
        ;lda #$00                    ; Set low byte to $EF if it underflowed
        ;sta COLOR_POS_LO
        dec COLOR_POS_HI 
        rts
